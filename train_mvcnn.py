...
from models.MVCNN import MVCNN, MVCNN2, SVCNN

parser = argparse.ArgumentParser()
parser.add_argument("-name", "--name", type=str, help="Name of the experiment", default="ElectronPhoton")
parser.add_argument("-bs", "--batchSize", type=int, help="Batch size for the second stage", default=48)# it will be *3 images in each batch for mvcnn
...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batchSize*3, shuffle=True, num_workers=0)
    ...
    create_folder(log_dir)
    cnet_2 = MVCNN2(args.name, cnet, nclasses=2, cnn_name=args.cnn_name, num_views=args.num_views)
    del cnet
